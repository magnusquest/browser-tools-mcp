...
async function getAvailablePort(startPort: number, maxAttempts: number = 10): Promise<number> {
...
app.post('/current-url', (req, res) => {
	...
	if (req.body && req.body.url) {
		...
		const timestamp = req.body.timestamp ? new Date(req.body.timestamp).toISOString() : 'unknown';
...
// Add endpoint to fetch the current active URL
app.get('/get-active-urls', (req, res) => {
	console.log('Fetching current active URL:', currentUrl);
	res.json({ activeUrl: currentUrl });
});
...
export class BrowserConnector {
	...
	constructor(app: express.Application, server: any) {
		...
		this.app.post('/capture-screenshot', async (req: express.Request, res: express.Response) => {
		...
		this.app.post('/screenshot', (req: express.Request, res: express.Response): void => {
			...
			const result = await new Promise((resolve, reject) => {
				...
				const messageHandler = (message: string | Buffer | ArrayBuffer | Buffer[]) => {
					try {
						...
						if (response.type === 'screenshot-data' && response.data && response.path) {
							// Remove the data:image/png;base64, prefix
